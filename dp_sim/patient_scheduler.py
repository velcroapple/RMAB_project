...
class WhittleCalculator:
    ...
    @staticmethod
    def verify_indexability(policy_matrix, lambdas, verbose=True):
        """
...
class Patient:
    def __init__(self, patient_id, p0, p1, rewards, beta=0.9):
        ...
        self.indices = WhittleCalculator.get_indices(self.P0, self.P1, self.R, self.beta)
        
        self.true_state = np.random.choice([0, 1])
        self.belief_sick = 0.5

class PatientScheduler:
    def __init__(self, patients, k=3, beta=0.9):
    ...
    def step(self):
        ...
        for p in self.patients:
            expected_w = (1 - p.belief_sick) * p.indices[0] + p.belief_sick * p.indices[1] #weighted whittle index based on belief
